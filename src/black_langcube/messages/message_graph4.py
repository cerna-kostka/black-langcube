...
async def message_graph4(language, subfolder, output_filename):
    ...
    try:
        ...
        raise RuntimeError(
            "Error getting recommendation value from graph4 output"
        ) from e

    # Build message based on recommendation and language.
    if recommendation == "stop":
        message = await compose_message(
            language, [graph4_text_stop], subfolder=subfolder / "message"
        )
        return message
    elif recommendation == "continue":
        message = await compose_message(
            language, [graph4_text_continue], subfolder=subfolder / "message"
        )
        return message
    else:
