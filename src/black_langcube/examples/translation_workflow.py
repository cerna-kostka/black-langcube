...
async def message_translator_en_fixed(language, subfolder, output_filename):
    """
    ...
    translation_out = await get_result_from_graph_outputs_async(
        "translator_en", "", "translation_output", "", subfolder, output_filename
    )
    tokens_out = await get_result_from_graph_outputs_async(
        "translator_en", "", "translation_tokens", "", subfolder, output_filename
    )
    message = translation_out, tokens_out
    return message


async def message_translator_usr_fixed(language, subfolder, output_filename):
    """
...
class TranslationWorkflow(BaseGraph):
    ...
    def __init__(
        ...
        super().__init__(
            TranslationWorkflowState, source_text, folder_name, target_language
        )
    ...
    def build_graph(self):
        ...
        def detect_translation_direction(
            state,
        ) -> Literal["translate_to_english", "translate_to_user_language"]:
        ...
        async def translate_to_english_node(state):
            ...
            return {"translated_text": translated_text, "translation_tokens": tokens}

        async def translate_to_user_language_node(state):
    ...
    async def run(self):
        ...
        events = await self.graph_streaming(self.state, recursion_limit=10)
        _subfolder = await self.write_events_to_file(events, self.output_filename)
        ...
        logger.info(
            f"--- Translation workflow completed. Result: {translated_text} ---"
        )
...
class CustomTranslatorEnSubgraf(TranslatorEnSubgraf):
    async def run(self, extra_input=None):
        ...
        message = await message_translator_en_fixed(
...
class CustomTranslatorUsrSubgraf(TranslatorUsrSubgraf):
    async def run(self, extra_input=None):
        ...
        events = await self.graph_streaming(extra_input or {}, recursion_limit=10)
        subfolder = await self.write_events_to_file(events, self.output_filename)
        message = await message_translator_usr_fixed(
            self.state.get("language"), subfolder, self.output_filename
        )
