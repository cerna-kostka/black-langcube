...
async def example_session_management():
    ...
    try:
        # Run a workflow
        _result = await run_workflow_by_id(
        ...
        cleanup_success = await cleanup_session(session_folder)
...
async def demonstrate_library_integration():
    ...
    class MyApplication:
        ...
        async def run_analysis_pipeline(self, query: str) -> dict:
    ...
    # Process a query
    query_result = await app.process_user_query(
    ...
    # Run an analysis
    analysis_result = await app.run_analysis_pipeline(
...
async def main():
    ...
    await demonstrate_library_integration()
