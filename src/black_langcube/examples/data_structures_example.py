...
def demonstrate_article():
    ...
    article = Article(topic="Artificial Intelligence in Healthcare", language="English")
...
def demonstrate_outline():
    ...
    items = [
        OutlineItem(
            foo="Introduction", baz="1", bar="Overview of the research problem"
        ),
        OutlineItem(
        ...
        OutlineItem(
            foo="Methodology", baz="3", bar="Research approach and methods used"
        ),
        OutlineItem(foo="Results", baz="4", bar="Key findings and data analysis"),
        OutlineItem(
            foo="Discussion", baz="5", bar="Interpretation of results and implications"
        ),
        OutlineItem(
            foo="Conclusion", baz="6", bar="Summary and future research directions"
        ),
    ]
    ...
    try:
        too_many_items = [
            OutlineItem(foo=f"Section {i}", baz=str(i), bar=f"Description {i}")
            for i in range(1, 11)
        ]  # 10 items, max is 8 so it will fail
        print("\nAttempting to create outline with too many items...")
        # This should raise a ValidationError
        _invalid_outline = Outline(items=too_many_items)
    except ValidationError as e:
...
def create_custom_data_structure():
    ...
    database = ResearchDatabase(
        papers=[paper1, paper2], total_count=2, search_query="AI in healthcare"
    )
