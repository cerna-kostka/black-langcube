...
Format your response as a structured analysis."""
    ...
    async def execute(self, extra_input=None):
        ...
        # Run the LLM chain
        result, tokens = await self.run_chain(extra_input or {})
...
Respond with only one word: 'positive', 'negative', or 'neutral'."""
        ...
        return [("system", system_prompt), ("human", text)]
...
class TextAnalysisWorkflow(BaseGraph):
    ...
    async def _log_and_execute(self, node_name, func, state):
    ...
    def build_graph(self):
        ...
        async def analyze_text_node(state):
            return await self._log_and_execute(
                "analyze_text", analyzer_node.execute, state
            )

        async def extract_summary_node(state):
            return await self._log_and_execute(
                "extract_summary", summary_node.execute, state
            )

        async def classify_sentiment_node(state):
            return await self._log_and_execute(
                "classify_sentiment", sentiment_node.execute, state
            )

        async def finalize_node(state):
            return await self._log_and_execute("finalize", finalize_analysis, state)

        # Add nodes to the graph
        self.add_node("analyze_text", analyze_text_node)
        self.add_node("extract_summary", extract_summary_node)
        self.add_node("classify_sentiment", classify_sentiment_node)
        self.add_node("finalize", finalize_node)
    ...
    async def run(self):
        ...
        # Await graph_streaming since it is now async
        await self.graph_streaming(initial_state)
...
async def main():
    ...
    for i, text in enumerate(sample_texts, 1):
        ...
        workflow = TextAnalysisWorkflow(
            input_text=text.strip(), folder_name=str(output_dir / f"analysis_{i}")
        )

        try:
            logger.info(f"Running workflow {i}...")
            _result = await workflow.run()
